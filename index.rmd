...
The `carbon` package has 2 dependencies
```r
...
...and `ppm_from_date` takes in a date, calls `get_weekly_data` and figrues out the closest ppm from a given date.
...
The function `ppm_from_date` is now exported for users.

---
## Dealing with dependencies

We have almost arrived at an installed package. We need to make sure that the dependencies of our package are properly specified. 

---
## Building and installing

We have placed code in the package and set up the function for exportation. We need to build and install the package to see if everything works well.
