...
---
## Let's make a package: `myebirdmap`

We are going to code a small package called `myebirdmap` which will produce an interactive map of an eBird data profile. 

The package is heavily inspired from an existing package that already allows you explore eBird data profile: the [`myebird`](https://github.com/sebpardo/myebird) package. Outside of a teaching context, our "ebird map" is a good example of a new feature that could be contributed to `myebird` instead of being the focus of another package (which allows me to give you an example of that).

The process we are going to go through touches on some aspect of RSE in eco evo:

1. loading and cleaning data 
2. Manipulating data into a form that we are interesting in 
3. Producing a visualization of such data

Due to time constraints, things like OOP will be outside the scope of this example.

---
## `myebirdmap` demo

```{r, fig.dim=c(5, 6), eval = TRUE}
library(myebirdmap)
my_ebird_map("data/raw_ebird_data.csv")
```

---
## The `myebirdmap` base script
...
```{r, eval = FALSE}
usethis::create_package("~/myebirdmap")
```

```{}
✓ Creating '/home/vlucet/myebirdmap/'
✓ Setting active project to '/home/vlucet/myebirdmap'
✓ Creating 'R/'
✓ Writing 'DESCRIPTION'
Package: myebirdmap
...
We are going to use the MIT license. It is important to get familair with licensing.
...
```{r, eval = FALSE}
usethis::use_r("my_ebird_map")
```

```{}
• Modify 'R/my_ebird_map.R'
• Call `use_test()` to create a matching test file
```

```{r, eval = FALSE}
usethis::use_r("process_ebird_data")
usethis::use_r("helper_functions.R")
```
